(function () {
	...
	class Triad {
	  constructor() {
	    ...
	    if (typeof options === 'string') {
	      ...
	      if (this.pitch == null) {
	        this.pitch = DEFAULTS$1.pitch;
	      }
	      if (this.acc == null) {
	        this.acc = DEFAULTS$1.acc;
	      }
	      if (this.type == null) {
	        this.type = DEFAULTS$1.type;
	      }
	  ...
	  accidentalSign() {
	    let forHTML = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
	    switch (this.acc) {
	      case -1:
	  ...
	  typeName() {
	    let forARIA = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
	    if (forARIA === true) {
	...
	class TriadPuzzle {
	  constructor() {
	    ...
	    document.body.addEventListener('touchend', event => {
	      ...
	      if (target.closest('.piece')) this.setupSheet();
	    });
	  }
	  setupSheet() {
	    let keepExisting = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
	...
	function applySettings(settings) {
	  ...
	  if (majorAndMinorToggle) {
	    majorAndMinorToggle.checked = settings.majorAndMinorOnly;
	  }
	  if (add9sToggle) {
	    add9sToggle.checked = settings.add9s;
	  }
	}
	function getStorage() {
